"""Quantum Neural Network combined with GRU (QNN-GRU) baseline (from notebook)."""
...
def _build_qnn_layer(n_qubits: int = 4, n_q_layers: int = 1, q_device: str = "default.qubit") -> qml.qnn.TorchLayer:
...
class QNNGRUModel(nn.Module):
    ...
    def __init__(
        ...
        self.input_projection = nn.Linear(input_dim, n_qubits) if use_projection else nn.Identity()
        self.qnn_layer = _build_qnn_layer(n_qubits=n_qubits, n_q_layers=n_q_layers, q_device=q_device)
